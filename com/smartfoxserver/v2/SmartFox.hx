	...
	*/
	...
	public function initUDP(manager:IUDPManager, udpHost:String=null, udpPort:Int=-1):Void
	{
		...
		if(isAirRuntime())
		{
			...
			*/
			if(!_bitSwarm.udpManager.inited && #if (haxe > "4.0.0") Std.isOfType #else Std.is #end(_bitSwarm.udpManager,DefaultUDPManager))
			{
				manager.sfs = this;
	...
	public function send(request:IRequest):Void
	{
		...
		try
		{
			...
			if(#if (haxe > "4.0.0") Std.isOfType #else Std.is #end(request, JoinRoomRequest))
			{
				if(_isJoining)
					return;
